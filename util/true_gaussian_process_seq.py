...
class true_GPPrior(torch.distributions.distribution.Distribution):
    ...
    def __init__(self, kernel=None, mean=None, lengthscale=None, var=None, nu=0.5, device='cpu', n_pos=None):
        ...
        base_cov = base_cov.to(torch.float64) # improve numerical stability for Cholesky decomposition
    ...
    def new_dist(self, n_pos): # understand if in my code this should be the role of x or y
        """
        Creates a Normal distribution at the points in x.
        n_pos: locations to query at, a flattened grid; tensor (n_points, dim)
        ...
        try:
            base_dist = torch.distributions.MultivariateNormal(base_mu.to(self.device), scale_tril = torch.linalg.cholesky_ex(base_cov.to(self.device))[0].to(torch.float32))
        except:
    ...
    def sample(self, n_pos, n_samples=1, n_channels=1):
        """ 
        Draws samples x0 from the GP prior.
        n_pos: torch.Tensor([n_points, x_dim])
        n_samples: number of samples to draw
        n_channels: number of independent channels to draw samples for (codomain dimension)
    ...
    def sample_from_prior(self, n_samples=1, n_channels=1):
        ...
        fixed prior
        """
        samples = self.base_dist.sample(sample_shapes = torch.Size([n_samples * n_channels, ]))
        samples = samples.reshape(n_samples, n_channels, -1)
        return samples
