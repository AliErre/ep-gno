...
class OTFuncFlowMatcherModel:
    ...
    @torch.no_grad()
    def sample(self, pos, query_pos, condition, n_channels=1, n_samples=1, n_eval=2, return_path=False, rtol=1e-5,
        ...
        x_0 = self.gp.sample(n_pos, n_samples=n_samples, n_channels=n_channels)

        def sample_func(t, x_t):
            return self.model(f_x=x_t, source_pos=pos,  query_pos=query_pos, time_condition=t, condition = condition) 
        out = odeint(sample_func, x_0, t, method=method, rtol=rtol, atol=atol)
